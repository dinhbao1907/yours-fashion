      ...
      if (design) {
        ...
        document.getElementById('likeHeart').addEventListener('click', async () => {
          ...
          if (liked) {
            ...
            await fetch(`http://localhost:5000/api/designs/${design.designId}/like`, { method: 'POST', headers: { 'Content-Type': 'application/json' } });
            ...
            await fetch(`http://localhost:5000/api/designs/${design.designId}/unlike`, { method: 'POST', headers: { 'Content-Type': 'application/json' } });
...
async function loadReviews(designId) {
  ...
  try {
    ...
    const response = await fetch(`http://localhost:5000/api/reviews?designId=${encodeURIComponent(designId)}`);
    ...
    let reviewCount = 0;
    try {
      const statsRes = await fetch(`http://localhost:5000/api/review-stats?designId=${encodeURIComponent(designId)}`);
      if (statsRes.ok) {
