document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (submitButton) {
    submitButton.addEventListener('click', async (e) => {
      ...
      try {
        console.log(`🌐 Making request to: http://localhost:5000${endpoint} [${method}]`);
...
function captureDesignImage() {
  ...
  return Promise.all(drawTasks).then(() => {
    try {
      ...
      alert('Không thể xuất ảnh thiết kế do lỗi bảo mật hình ảnh. Một số ảnh ngoài không hỗ trợ xuất.');
      return null;
    }
  });
}